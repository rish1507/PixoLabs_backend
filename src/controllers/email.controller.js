...
const processBatchWithDelay = async (items, processFunction, delayMs = 1000) => {
  const results = [];
...
exports.generateEmailReply = async (req, res) => {
  try {
      const { instructions, emailId, subject, originalBody,from,sender } = req.body;
      ...
      const completion = await openai.chat.completions.create({
          ...
          messages: [
              {
                  role: "system",
                  content: "You are a helpful assistant that generates professional email replies and keep email concise. and not include any placeholder only generate general email."
              },
              {
                  role: "user",
  ...
  try {
      ...
      const completion = await openai.chat.completions.create({
          ...
          messages: [
              {
                  role: "system",
                  content: "You are a helpful assistant that edits emails based on instructions."
              },
              {
                  role: "user",
